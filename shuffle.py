...
def ilmpp_prover(p : int, q : int, X : List[int], Y : List[int], x : List[int], y : List[int], gamma : int) -> Tuple[List[int], List[int]] :
  k = len(X)
  assert len(Y) == k
  assert len(x) == k
  assert len(y) == k
...
def ilmpp_verifier(p : int,  X : List[int], Y : List[int], A : List[int], r : List[int], gamma) -> bool: 
  k = len(X)
  assert len(Y) == k
  assert len(A) == k
  assert len(r) == (k - 1)
...
A, r = ilmpp_prover(p, q, X, Y, x, y, gamma)
#print(x, y, X, Y, A, r)
print(ilmpp_verifier(p, X, Y, A, r, gamma))
